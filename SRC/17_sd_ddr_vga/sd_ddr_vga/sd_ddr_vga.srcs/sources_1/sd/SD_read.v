...
parameter SADDR=32'd8256;        //第一副图片在SD卡的起始sector地址8192+11328
...
if(!init)
	...
	case(mystate)
		idle:	begin
				...
				SD_datain<=1'b1;
				cnt<=22'd0;	
				if((read_o==1'b0) && (ddr_init_done==1'b1))        //如果还没有把SD卡的图片存放到SDRAM中
					begin
						mystate<=read;
						read_o<=1'b0;
...
begin
	if(!init)begin
		...
		2'b01: begin             //接收512个bytes
			if(read_cnt<16)	begin     //读取512字节, 16个256bit数据
			   if(cntb<255)	begin
				      myvalid_o<=1'b0;
					   mydata<={mydata[254:0],SD_dataout};
					   cntb<=cntb+1'b1;
						data_come<=1'b0;
				 end
				 else begin
						myvalid_o<=1'b1;            //一个256位数据有效
						mydata_o<={mydata[254:0],SD_dataout};
						cntb<=0;
