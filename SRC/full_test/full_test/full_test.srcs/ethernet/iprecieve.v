  		   ...
  		   rx_data: begin                                             //接收UDP的数据       
					if(e_rxdv==1'b1) begin
					   if (data_counter==rx_data_length-9) begin         //存最后的数据,真正的UDP数据需要减去8字节的UDP包头
						    data_counter<=0;
  					       rx_state<=rx_finish;
							 ram_wr_addr<=ram_wr_addr+1'b1;	  //RAM地址加1
							 data_o_valid<=1'b1;               //写RAM 							 
						    if(byte_counter==3'd3) begin                        //满32bit, 不需补0
								  data_o<={mydata[23:0],datain[7:0]};
								  byte_counter<=0;
							 end
						    else if(byte_counter==3'd2) begin
						        data_o<={mydata[15:0],datain[7:0],8'h00};       //满24bit, 不满32bit,补8位0
								  byte_counter<=0;
							 end
						    else if(byte_counter==3'd1) begin
						        data_o<={mydata[7:0],datain[7:0],16'h0000};     //满16bit, 不满32bit,补16位0
								  byte_counter<=0;
							 end
						    else if(byte_counter==3'd0) begin
						        data_o<={datain[7:0],24'h000000};              //满8bit, 不满32bit,补24位0
								  byte_counter<=0;
							 end
						end
						else begin
